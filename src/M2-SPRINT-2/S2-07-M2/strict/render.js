...
function renderizaNovoProduto(produto) {
    ulTag.insertAdjacentHTML('beforeend', `
        <li id=${`produto-${produto.id}`}>
            ...
            <button onclick="removeProdutoDoCarrinho(event)" id=${produto.id}>X</button>
        </li>
    `)
}
...
function adicionaProdutoAleatorioAoCarrinho() {
    ...
    const produtoAleatorioEstaNoCarrinho = listaDoCarrinho.some(produto => produto.id === indexAleatorio)
...
function removeProdutoDoCarrinho(event) {
    ...
    const produtoNoCarrinho = listaDoCarrinho.find(produto => produto.id == idProdutoNaDOM)
    ...
    if(produtoRemovido === produtoNoCarrinho) {
        ...
        console.error("Algo não esta funcionando corretamente, o produto ainda existe no array listaDoCarrinho ou o retorno da função esta incorreto", listaDoCarrinho)
    }
}
...
function renderizaProdutosIniciais(carrinho) {
    carrinho.forEach(produto => {
        ...
        <li id=${`produto-${produto.id}`}>
            ...
            <button onclick="removeProdutoDoCarrinho(event)" id=${produto.id} class="remove">X</button>
        </li>
        `)
    })
}
